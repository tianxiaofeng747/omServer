...
Select.prototype.render = function () {
    var me = this;
    var currentoption = me.options[me.pointerPosition].text;
    ...
    me.options.forEach(function(option, position){
        ...
        if (position === 0) {
            console.log(color.cyan(option.text) + ' ' + color.white(currentoption));
        } else if (prefix.trim()) {
            console.log(color[me.config.checkedColor](prefix + option.text));
        } else {
            console.log(color.white(prefix + option.text));
        }
...
Select.prototype.prev = function () {
    this.uncheckoption();
    this.pointerPosition -= ( this.pointerPosition > 1 ) ? 1 : 0;
    this.checkoption();
    this.changeSelected();
};
...
Select.prototype.selectoption = function () {
    var r = this.config.multiSelect ? this.optionsSelected : (this.optionsSelected[0] || this.options[1]);
